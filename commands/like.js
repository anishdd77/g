...
let LUser;
  ...
  if(message.mentions.users.size < 1) return message.channel.send("**`#قلب @user#123`**").then(msg => {msg.delete(6000)});
  let rows = sql.prepare(`SELECT * FROM profile WHERE UserID = '${pUser.id}'`).get()
    ...
    let LUser  = rows.likes;
           ...
           sql.prepare(sqlstr).run()
            ...
            sql.prepare(sqlstr).run()
             return message.reply("**تم إعطائه قلب**").then(msg => {msg.delete(5000)});
